...
export async function recordTopicView(topicId: string, ipAddress?: string) {
  const supabase = await createServerSupabaseClient();
  ...
  try {
    ...
    // Use the optimized database function with built-in rate limiting
    const { error } = await (supabase as any).rpc('increment_topic_view', {
      p_topic_id: topicId,

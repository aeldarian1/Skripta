...
CREATE POLICY "Authors can update own topics"
  ...
  USING (auth.uid() = author_id OR auth.is_admin());
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_topic_views_topic_ip
...
RAISE NOTICE 'Optimizations applied successfully!';
RAISE NOTICE 'Review verification queries above.';
RAISE NOTICE 'Monitor query performance in Supabase Dashboard > Database > Query Performance';
